...
function toggleFavorite(place) {
  ...
  axios
    .post(`/api/favorites/${place.data.name}/place`, {
...
function getCurrentWeather(lat, lng) {
  return axios.get(`/api/place?lat=${lat}&lng=${lng}`);
}

async function buildFavoritesTopbar() {
  const places = await fetchFavorites();
...
function loadPlaces(map, place) {
  ...
  axios.get(`/api/place?lat=${lat}&lng=${lng}`).then(res => {
    if (!res) {
      alert("Cant't fetch data!");
      return;
    }
