...
// console.log(document.querySelector('.map__container'));

(0, _map.makeMap)(document.querySelector('#map__background'));

(0, _map.fetchInitialListOfFavorites)();
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(137), __webpack_require__(359)(module)))
...
exports.fetchInitialListOfFavorites = undefined;

var fetchInitialListOfFavorites = exports.fetchInitialListOfFavorites = function () {
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            fetchedPlaces = void 0;


            _axios2.default.get('/api/places').then(function (userPlaces) {
              fetchedPlaces = userPlaces.data || [];
              console.log(fetchedPlaces);
              if (!fetchedPlaces) return;

              return fetchedPlaces.map(function (pl) {
                console.log('drukuje', pl.location.coordinates);
                _axios2.default.get('/api/place?lat=' + pl.location.coordinates[0] + '&lng=' + pl.location.coordinates[1]);
              });
            }).then(function (favoritePlaces) {
              console.log(favoritePlaces);
              buildInitialListOfFavorites(favoritePlaces);
            });
...
function toggleFavorite(place) {
  ...
  _axios2.default.post('/api/place/favorite', { lat: place.data.coord.lat,
    ...
    var alertBox = document.querySelector('.map__alert');
...
function buildInitialListOfFavorites(places) {
  // if (places.length === 0 || places) return [];
  var favorites = document.querySelectorAll('.map__favorites');
  console.log('build', places);
  var html = '';

  places.data.forEach(function (place) {
    html += '\n      <article class=\'map__favorite\'>\n        <img src=\'http://openweathermap.org/img/w/' + (place.icon || 'abc') + '.png\' class=\'map__img\' alt=\'' + place.name + '\' title=\'' + place.name + '\'>\n        <h3>' + place.name + '</h3>\n        <p>AAA</p>\n      </article>\n    ';
  });
...
function loadPlaces(map, place) {
  ...
  var lat = place.geometry.location.lat() || mapOptions.center.lat;
  var lng = place.geometry.location.lng() || mapOptions.center.lng;
